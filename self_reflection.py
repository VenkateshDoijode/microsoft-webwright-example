...
REFLECTION_PROMPT = """
You are a critical evaluator reviewing the output of a web automation agent.
...
Be concise and factual. Flag any missing data, hallucinated results,
or incomplete steps.
"""
...
def reflect(task: str, report: str, log_entries: list) -> dict:
    ...
    if not log_entries:
        print(
            "[Self-Reflection] Warning: no log entries provided "
            "- reflecting on report only."
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"Self-reflection API call failed: {e}"
        ) from e
    ...
    if not raw or not raw.strip():
        raise RuntimeError(
            "Self-reflection returned an empty response."
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to parse self-reflection response: {e}\nRaw: {raw}"
        ) from e
...
def run_reflection(
    ...
    """
    Load report and log, run reflection,
    ...
    try:
        ...
        print(
            f"[Self-Reflection] Warning: "
            f"could not save result file: {e}"
        )

    print(
        f"[Self-Reflection] Status: "
        f"{result.get('overall_status', 'unknown')}"
    )
    print(
        f"[Self-Reflection] Result saved to: {output_path}"
    )
